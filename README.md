...
- 手动或代抽时，牌先背面朝上落位，全部到齐后同步翻开，再开始 AI 解读。
...
**作者自用的塔罗牌牌面图案因版权限制无法开源，故不随本项目上传或发布，用户下载本项目自用可根据自己喜好更换塔罗牌牌面图案。** 开源版保留程序化粒子牌面，下载代码后即可正常抽牌，不需要另外准备图片。项目代码的 ISC 许可不覆盖用户自行添加的第三方图片。
